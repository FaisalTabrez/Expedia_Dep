...
| M3.3 | Execute reference experiments | M3.2 | Immutable run manifests, raw outputs, environment/provenance records, integrity digests | **In progress:** the evidence-only M3-001 runner mechanically verifies the approved environment and release before isolated replicates; it imports Query Core only from the frozen implementation workspace. No execution evidence has been generated yet. Every run is bounded by the preregistration and deviations are retained. | XL |
