...
| M3-002.6 | Execute comparison | **Authorized, pre-execution restart pending:** both retained Git incidents are addressed only by the approved command-scoped trust amendment for the single detached frozen workspace; the full pre-execution sequence restarts before any release, Query Core, oracle, or comparison work. |
