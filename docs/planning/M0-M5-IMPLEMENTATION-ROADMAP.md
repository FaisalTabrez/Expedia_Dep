...
| M3 — Method evaluation | **Planned.** Produce evidence, not feature demonstrations. | M1/M2; accepted OQ-05 and an accepted study preregistration; OQ-04/OQ-08 only if their scope is proposed. ADR-009/014 remain Proposed EDS entries for public/default-method promotion. | Preregistered studies, versioned cohorts/baselines, exact ground truth where applicable, raw result artifacts, uncertainty/failure analysis, decision ADRs. | Each claim is scoped to its preregistered evidence. No default profile/index/bridge/graph method is promoted without required baselines and review. |
...
| OQ-05: benchmarks for allowable usefulness claims | Resolved for M3: accepted claim-evidence requirements and preregistration policy | M3 study execution and any claim review |
