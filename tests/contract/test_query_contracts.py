...
class QueryContractTests(unittest.TestCase):
    def test_formatting_member_order_and_default_pagination_do_not_change_digest(self) -> None:
        request = _request({"in": {"field": "record_id", "values": ["b", "a"]}})
        left = canonicalize_query_request_json(json.dumps(request, indent=2))
        reordered = {
            "similarity": request["similarity"],
            "operation": request["operation"],
            "filter": {"in": {"values": ["a", "b"], "field": "record_id"}},
    ...
    def test_boolean_filter_order_is_semantic_order_independent(self) -> None:
        left = canonicalize_query_request_json(json.dumps(_request({"all": [{"eq": {"field": "record_id", "value": "a"}}, {"eq": {"field": "lifecycle_state", "value": "active"}}]})))
        right = canonicalize_query_request_json(json.dumps(_request({"all": [{"eq": {"value": "active", "field": "lifecycle_state"}}, {"eq": {"value": "a", "field": "record_id"}}]})))
        self.assertEqual(left.digest, right.digest)

    def test_equivalent_numeric_spellings_have_one_canonical_interpretation(self) -> None:
        left = canonicalize_query_request_json(json.dumps(_request({"range": {"field": "record_id", "lower": 1.0, "upper": 2.00, "unit": "ordinal"}})))
        right = canonicalize_query_request_json(json.dumps(_request({"range": {"field": "record_id", "lower": 1, "upper": 2, "unit": "ordinal"}})))
        self.assertEqual(left.digest, right.digest)
    ...
    def test_duplicate_semantic_filter_values_are_deduplicated(self) -> None:
        duplicated = canonicalize_query_request_json(json.dumps(_request({"in": {"field": "record_id", "values": ["a", "a"]}})))
        unique = canonicalize_query_request_json(json.dumps(_request({"in": {"field": "record_id", "values": ["a"]}})))
        self.assertEqual(duplicated.digest, unique.digest)

    def test_filter_cost_limits_are_enforced(self) -> None:
        too_many = {"all": [{"eq": {"field": "record_id", "value": str(index)}} for index in range(33)]}
        with self.assertRaisesRegex(QueryContractError, "32 predicate"):
            canonicalize_query_request_json(json.dumps(_request(too_many)))

        nested: object = {"eq": {"field": "record_id", "value": "a"}}
        for _ in range(8):
            nested = {"not": nested}
        ...
        with self.assertRaisesRegex(QueryContractError, "membership exceeds 100"):
            canonicalize_query_request_json(json.dumps(_request({"in": {"field": "record_id", "values": list(map(str, range(101)))}})))

        with self.assertRaisesRegex(QueryContractError, "exceeds 16 KiB"):
            canonicalize_query_request_json(json.dumps(_request({"eq": {"field": "record_id", "value": "x" * 17_000}})))
